...
int Chessboard::is_check(bool in_black, int st_y, int st_x, int end_y, int end_x)
{
   ...
   bool mate = (board[end_y][end_x]->is_white() == in_black);
   ...
   if(mate)//La mossa e' d'attacco
   {
       ...
       if (board[end_y][end_x]->is_valid_move(board, end_y, end_x, k_y, k_x))
       {
           found = true;
...
bool Chessboard::is_checkmate_s(int k_y, int k_x, std::pair<int,int> t_pos, bool in_black)
{
   ...
   for(int x = 0; x < 8; x++)
   {
       for(int y = 0; y < 8; y++)
       {
           ...
           {
               for(int t_x = k_x + dir_x; in_bounds(t_x + dir_x) ; t_x = t_x + dir_x)
               {
                   for(int t_y = k_y + dir_y; in_bounds(t_y + dir_y); t_y = t_y + dir_y)
                   {
...
bool Chessboard::is_draw(int end_y, int end_x )
{
   ...
   for(int y = 0; y < 8; y++)
   {
       for(int x = 0; x < 8; x++)
       {
                   ...
                   for(int off_y = -2; off_y <= 2; off_y++)
                    {
                        ...
                        if(off_y%2 == 0)
                        {
                            for(int off_x = -1; off_x <= 1; off_x+=2)
                            {
                                ...
                                {
                                    if(!implies_check(y, x, y +off_y, x +off_x))
                                        return false;
                                }
                            }
                        }
            ...
            //std::cout << "Non ci sono mosse possibili \n";
       }
   }
